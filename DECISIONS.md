...
## Feature — Saarthi Support (scope-locked assistant with auto-remediation)

| # | Decision | Reasoning |
|---|---|---|
| P.1 | Provider-neutral LLM client, not a vendor-named one | The key supplied was a **Groq** key (`gsk_…`), not an xAI **Grok** key (`xai-…`) — different companies, one letter apart. Since both are OpenAI-compatible, the client takes a base URL and model ids from env, so switching providers is a `.env` edit. A file called `grok.client.ts` talking to Groq would have been a permanent trap. |
| P.2 | `openai/gpt-oss-120b` for reasoning | It was the only available model supporting strict `json_schema`; `llama-3.3-70b` rejects it outright. Structured output is the whole of scope-locking layer 2, so this constraint chose the model. |
| P.3 | `args` travels as a JSON *string* | Strict schema mode requires `additionalProperties:false` on every object, which cannot express a free-form argument map. The server parses the string and validates it against the action's own zod schema — stricter than the alternative, since those arguments were always going to be re-validated. |
| P.4 | Postgres full-text retrieval, no vector database | ~130 chunks. `ts_rank_cd` over a GIN index is accurate enough, adds no infrastructure, and needs no embedding API — which is what keeps the demo working with the network unplugged. An embedding service would have made an offline requirement depend on being online. |
| P.5 | Document-frequency pruning before ranking | Postgres full-text has no IDF, so in an OR-query a corpus-wide word like "task" outweighs the rare word that identifies the problem. Dropping terms found in >40% of documents fixed "cannot create a critical task in **health**"; normalisation flags (`ts_rank_cd` 0/1/2/32) made no measurable difference at all. |
| P.6 | Errors are ranked by term **coverage**, features by **rank** | Not taste — the corpora differ. Error documents share vocabulary ("code", "email", "account"), so repetition misleads and only the count of distinct discriminating terms separates them: "tell me today's gold **rate**" scores 4.20 against RATE_LIMITED, higher than a genuine report scores against its own document. Feature documents each describe a different screen, so they are already distinct and rank alone is decisive. One rule for both leaked in one direction or over-refused in the other. |
| P.7 | The model must fill `questionSubject` before deciding scope | Judging the whole sentence failed the wrapper trick: "In SMARTWORK 360, who is the prime minister?" was answered because retrieval found nearby text. Forced to name the subject first — "Indian politics" — the verdict becomes obvious. A server-side regex over that field backstops it. |
| P.8 | The offline path needs its own scope check | It had none, and would answer anything from whichever document ranked highest. Not hypothetical: the provider rate-limited mid-test and **every** request landed there. It now needs a rank floor, a coverage floor, and an off-topic pre-check, and refuses otherwise — a confident wrong answer is worse than "I don't know". |
| P.9 | Offline fix arguments come from the server's error record | The browser's copy of an error is editable by whoever is sitting in front of it, and it is about to be fed to a model. Reading our own record is both safer and more accurate — and it is why auto-fix works offline at all, since the ids never came from generation. |
| P.10 | No "high" risk tier in the registry | An action too dangerous for a one-click confirm does not belong in an assistant. Rather than gate it, it is absent: nothing touching AuditEvent/Anchor rows, no deletions, no role or password changes, no bulk edits. Absence is a stronger guarantee than a permission check, and it is why the chain-repair refusal cannot be misconfigured. |
| P.11 | `resolveSlaHours` now refuses instead of defaulting | It silently fell back to a hidden global default, so task creation never actually failed and the headline demo would have been staged. A deadline is what every SLA figure is measured against; inventing one produces reports that look precise and mean nothing. |
| P.12 | `User.departmentId` became nullable | Required for the USER_NO_DEPARTMENT scenario. **The security-relevant part:** passing an undefined departmentId to Prisma DROPS the filter rather than narrowing it, so a manager with no department would have seen every department instead of none. The scope helpers now fail closed through a sentinel that matches no row. |
| P.13 | Callers inside a modal close themselves before opening support | A Radix modal traps pointer events across the page, so "Ask Saarthi about this" inside the task dialog opened a panel that was visible and impossible to type into. Found by clicking it, not by typechecking. |
| P.14 | The chat schema accepts `null` as "absent" | The panel sends `correlationId: null` when nothing is attached — the common case — and `z.string().optional()` rejects null, so every such question failed with a 422. It passed early testing only because the tests always had an error attached. |
| P.15 | Admins may use the manager section, and pick a department | An admin is a superset of a manager and raises work anywhere, but the section was locked to MANAGER and the new-task modal hard-coded the caller's own department — leaving admins with no task-creation screen at all. |
| P.16 | The adversarial suite is committed, not ad-hoc | `apps/api/scripts/support-adversarial.ts` asserts the five properties that must survive any future change to the prompt, the knowledge base or the model, plus the full remediation loop. It passes 39/39 in **both** model and offline mode, and is the thing to run before trusting a change to any of them. |

## Fix — the guided tour got stuck on cross-page action steps

| # | Decision | Reasoning |
|---|---|---|
| S.11 | Action steps perform their own `nextRoute` navigation | NextStep only handles `nextRoute` inside its own Next button, and its context exposes no equivalent method. An action step advances through `setCurrentStep`, skipping that handler — so a step with **both** an action and a route advanced the index while leaving the user on the old page, spotlighting a target that only exists on the new one. Four steps were affected. The step registry now carries `nextRoute` so the action path can navigate before advancing. |
...
## Feature — Brand identity wired across the product

| # | Decision | Reasoning |
|---|---|---|
| B.1 | One `<BrandLogo>` component; no raw `<img src="/brand/...">` anywhere | Theme becomes a prop rather than a per-call-site guess. The whole class of "white mark on a white background" bugs disappears at the point where it would otherwise be introduced. |
| B.2 | The wordmark is HTML text, never inside the SVG | It stays selectable, screen-reader readable, translatable and restyleable, and it cannot go blurry at any size. The SVG file holds the mark alone. |
| B.3 | `og-image.png` was composed, not supplied | It was the one asset missing from the set. Composed in headless Chromium around the **unmodified** `mark-white.svg`, so the mark itself is untouched — the brief allows rasterising from the existing SVG, and criterion 2 requires the file. |
| B.4 | A white raster, `mark-white-512.png`, was added for email | The brief said to put `mark-512.png` on the navy header band, but that file is the **navy** mark: on navy the S disappears and only three orange squares survive. Rasterised the white SVG instead and checked it against the real band colour. |
| B.5 | `PUBLIC_WEB_URL` falls back to the existing `APP_BASE_URL` | Two variables that must always hold the same value are a bug waiting to happen: when they drift, the sign-in link in an email keeps working while the logo silently breaks — which nobody notices until a real user reports it. |
| B.6 | Parichay keeps its own "GoI" monogram | The clean-up grep flags it, but that mark belongs to the **mock identity provider**, not to us. Putting the SMARTWORK mark in that masthead would make a simulation of somebody else's SSO look like our own page, which undercuts the honest sandbox framing the rest of the project maintains. Ours sits subordinate beside the sandbox badge. |
| B.7 | The print header is a real element, not only a CSS `content:` swap | The brief's `content: url(...)` trick is included, but it only helps for marks that remain visible; the sidebar carrying the identity is hidden when printing. A dedicated print-only masthead with the mono mark, the title and a generated-at timestamp is what actually makes a printed page traceable. |
| B.8 | Printing hides the sidebar and topbar and reclaims the offset | Chrome drops backgrounds by default, so the navy sidebar prints as a white column of invisible white text and the content stays indented past it. |
| B.9 | CSV exports get a provenance comment, never an image | A logo in a spreadsheet is noise, and half the tools that open these files would render it as a broken cell. What a file leaving the building needs is to say where it came from and when. |
| B.10 | `/public/icons/*.svg` deleted and the service worker precache updated | They were superseded by the real PNG icons and nothing referenced them any more — except the service worker, which was still precaching a file that no longer existed. Cache name bumped so returning users pick up the change. |
